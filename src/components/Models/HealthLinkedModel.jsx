...
const HealthLinkedModel = ({
    ...
    const DeviceConn = () => {
        return (
            <View style={{ marginTop: 37 }}>
                <Image source={connection} style={{ alignSelf: "center", width: 80, height: 80, }} resizeMode='contain' />

                <Text className="text-[24px] leading-[25px] text-center font-normal text-black px-1 font-Larken font-[400] mb-[10px] mt-[24px]">
                    {headerText}
                </Text>
                <Text style={{ fontSize: 14, fontWeight: '400', textAlign: 'center', lineHeight: 16, marginHorizontal: 40, marginTop: 16, marginBottom: 80 }}>{"Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."}</Text>
            </View>
    ...
    const DeviceConnError = () => {
        return (
            <View style={{ marginTop: 37 }}>
                <Image source={connection_error} style={{ alignSelf: "center", width: 80, height: 80, }} resizeMode='contain' />
    ...
    return (
        <ModelBox isVisible={isModelOpen} onClose={hanldeCloseModel}>
            <View className='bg-[#FFFCFB] bottom-0 w-full rounded-t-[40px]' style={{ paddingBottom: 70, alignSelf: 'center' }}>
                ...
                <NextButton onPress={() => onButtonHandle()} title={ScreenType == 0 ? "Start syncing" : "Try linking again"} isContinueBtn={true} />
            </View>
        </ModelBox>
    );
}
