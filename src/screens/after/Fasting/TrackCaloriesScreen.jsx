    ...
    const TrackCaloriesRenderItem = ({ item, index }) => {
        return (
            <View
                ...
                <Text style={{ fontSize: 24, fontWeight: '700', fontFamily: 'Larken', lineHeight: 25.44 }}>{"Meal: " + item.key}</Text>
                <Text style={{ fontSize: 12, fontWeight: '400', lineHeight: 14.18, marginBottom: 20 }}>{item?.time}</Text>

                <Image source={item.img} style={{ width: "100%", height: 180, borderRadius: 30 }} />
                {
                    item.like && <TouchableOpacity style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: "#FFEDE0", position: "absolute", right: 20, top: 70, alignItems: 'center', justifyContent: 'center' }}>
                        <Image source={heart_Icon} style={{ width: 20, height: 17, borderRadius: 30, }} />
                    </TouchableOpacity>
                }

                <View className='px-4 py-2 pb-4'>
                    <Text className='text-[12px] text-black leading-[16.8px] font-[600]'>{item.name}</Text>
                    <View className='flex flex-row items-center mt-[8px]'>
                        <Text className='text-[10px] text-[#FE7701] leading-[16.8px] font-[600]'>{item.kcal}</Text>
                        <View className='mx-2 h-[4px] w-[4px] rounded-full bg-[#18192B]/50'></View>
                        <Text className='text-[10px] text-[#18192B]/50 leading-[16.8px] font-[400]'>{item.type}</Text>
                    </View>
                </View>
            </View>
        )
...
const TrackCaloriesScreen = () => {
    ...
    const [isLogMealModelOpen, setIsLogMealModelOpen] = useState(false);
    ...
    const DailyView = () => {
        return <ScrollView style={{ flex: 1, width: '100%' }} showsVerticalScrollIndicator={false}>
            <View style={{ flex: 1 }}>
                ...
                <Text style={{ fontSize: 24, fontWeight: "700", fontFamily: 'Larken', marginHorizontal: 30, color: '#FE7701', lineHeight: 25.44 }}>{"298 kcal "}
                    <Text style={{ color: '#000', fontWeight: "400", }}>{"consumed"}</Text>
                </Text>
                <Text style={{ fontSize: 12, fontWeight: "400", lineHeight: 14.18, marginHorizontal: 30, color: '#18192B' }}>{"Data from your eating logs"}</Text>
                        ...
                        <View>
                            {
                                index == ChartData?.datasets[0]?.data.length -2 && <ImageBackground key={index}
                                   ...
                                   <Text style={{ fontSize: 16, color: '#000' , fontFamily:'Larken' , fontWeight:'700' , fontStyle:'italic' }}>{indexData+" cal"}</Text>
                                </ImageBackground>
                            }
                        </View>
                ...
                <View style={{ flexDirection: 'row', marginHorizontal: 30 }}>
                    <Text style={{flex:1, fontSize: 12, fontWeight: '700', lineHeight: 14.4 }}>{"3.00"}
                ...
                <View className={`flex bg-white  mt-4 w-[330px] px-[24px] py-[24px] rounded-[30px] mb-2 border border-gray-100 shadow-sm shadow-slate-300 sha overflow-hidden`} style={{alignSelf:'center'}}>
                    <View className="flex flex-row justify-between items-center">
                        <View className="flex flex-row gap-x-2 justify-center items-center">
                            <Image source={require('../../../assets/afterscreen/home/kcal.png')} className="h-[40px] w-[36.26px]" />
                            <View className="flex justify-start items-start">
                                <Text className="text-black text-base font-semibold leading-[0.1]">Calories</Text>
                                <Text className="text-gray-600 text-[10px] leading-[0.1] -mt-1">Sun, 15th Nov</Text>
                            </View>
                        </View>
                    ...
                    <View className='flex flex-row items-center align-middle justify-between mt-[41px] mb-3'>
                        <View className='flex flex-row items-center gap-x-2'>
                            <View className='flex'>
                                <Text className='text-[12px] text-[#18192B]/80 font-[400] leading-[14.18px]'>Intake</Text>
                                <Text className='text-[12px] text-[#18192B] font-[700] leading-[14.18px] '>1,100 kcal</Text>
                            </View>
                            <View className="border h-[15px] border-[#18192B]/80 bg-[#18192B]/80"></View>
                            <View className='flex'>
                                <Text className='text-[12px] text-[#18192B]/80 font-[400] leading-[14.18px]'>Required</Text>
                                <Text className='text-[12px] text-[#18192B] font-[700] leading-[14.18px] '>1,700 kcal</Text>
                            </View>
                        </View>
                        <View className="flex justify-center items-end align-middle">
                            <Text className='text-[12px] text-[#18192B]/80 font-[400] leading-[14.18px]'>Recommended</Text>
                            <Text className='text-[12px] text-[#18192B] font-[700] leading-[14.18px] '>2,000 kcal</Text>
                        </View>
                    </View>
                    <Progress.Bar progress={45 / 100} borderRadius={4} width={282} height={4} className="bg-[#FFEDE0] mt-3 mx-auto" borderWidth={0} color="#FF9950" />
                </View>

                <View className="flex flex-row mt-[8px]" style={{alignSelf:'center'}}>
                    {eat_CabsList.map((item, index) => {
                        return (
                            <View key={index} className={`flex w-[104.67px] h-[80px] border border-[${item.color}]/30 rounded-[16px] flex-row mr-[8px]`} style={{
                                borderColor: item.color,
                            }}>
                                <View className={`items-center justify-center bg-[${item.color}]/10 rounded-l-[16px] w-[30px] h-[78px]`}
                                 style={{backgroundColor:item.bg , }}>
                                    <Text className={`-rotate-90 leading-[14px] text-[10px] font-[700] w-[80px]`} style={{
                                        color: item.color,
                                        textAlign:'center'
                                ...
                                <View className="justify-center items-start ml-[8px] py-[14px]">
                                    ...
                                    <Progress.Bar progress={45 / 100} borderRadius={4} width={50} height={4} className="bg-[#FFEDE0] mt-3 mx-auto" borderWidth={0} color={item.color} />
                                </View>
                            </View>
                        )
                    })}
    ...
    const WeeklyView = () => {
        return <ScrollView style={{ flex: 1, width: '100%' }} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
            <View style={{ flex: 1 }}>
                ...
                <Text style={{ fontSize: 24, fontWeight: "700", fontFamily: 'Larken', marginHorizontal: 30, color: '#FE7701', lineHeight: 25.44 }}>{"14,588 kcal "}
                    <Text style={{ color: '#000', fontWeight: "400", }}>{"consumed"}</Text>
                </Text>
        ...
        <Background>
            <View style={{ flex: 1 }}>
                <View style={{ flexDirection: "row", marginTop: 70, marginHorizontal: 24, alignItems: 'center' }}>
                    ...
                    <Text style={{ fontSize: 20, fontWeight: '500', fontFamily: 'Larken', marginHorizontal: 16, flex: 1 }}>{"Track calories"}</Text>
                </View>
